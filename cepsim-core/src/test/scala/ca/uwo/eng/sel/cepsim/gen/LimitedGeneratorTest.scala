...
class LimitedGeneratorTest extends FlatSpec
  ...
  "A LimitedGenerator" should "limit tuples generated by a generator" in {
    ...
    val gen = new Generator() {
      override def doGenerate(): Double = 100
      override def samplingInterval: Long = 100
    }

    gen.generate(50) should be (50.0)
  }

  it should "not lose any generated event" in {
    val gen = new Generator() {
      override def doGenerate(): Double = if (invocations == 0) 90 else 20
      override def samplingInterval: Long = 100
    }
  ...
  it should "return the generated value if it is not over the limit" in {
    val gen = new Generator() {
      override def doGenerate(): Double = if (invocations == 0) 40 else 20
      override def samplingInterval: Long = 100
    }

    gen.generate(50) should be (40.0)
    gen.generate(50) should be (20.0)
    gen.nonProcessed should be ( 0.0)
  }

  it should "correctly calculate the average generation rate" in  {
    val gen = new Generator() {
      override def doGenerate(): Double = if (invocations == 0) 60 else 20
      override def samplingInterval: Long = 100
    }

    gen.generate(50) should be ( 50.0)
    gen.generate(50) should be ( 30.0)
    gen.nonProcessed should be (  0.0)
