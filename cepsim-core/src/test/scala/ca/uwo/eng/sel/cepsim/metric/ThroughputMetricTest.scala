...
class ThroughputMetricTest  extends FlatSpec
  ...
  "A ThroughputMetricCalculator" should "calculate the right throughput for one iteration" in new Fixture1 {
    ...
    throughput update Generated(prod1,  0.0, 10.0, 10.0)
    throughput update Produced (prod1, 10.0, 11.0, 10.0, Map.empty)
    throughput update Produced (op1,   11.0, 21.0, 10.0, Map(prod1 -> 10.0))
    throughput update Produced (op2,   21.0, 22.0,  1.0, Map(op1   ->  1.0))
    throughput update Consumed (cons1, 22.0, 23.0,  1.0, Map(op2   ->  1.0))
  ...
  it should "calculate the right throughput for two iterations" in new Fixture1 {
    ...
    throughput update Generated(prod1,  0.0, 10.0, 30.0)
    throughput update Produced (prod1, 10.0, 11.0, 30.0, Map.empty)
    throughput update Produced (op1,   11.0, 21.0, 30.0, Map(prod1 -> 30.0))
    throughput update Produced (op2,   21.0, 24.0,  3.0, Map(op1   ->  3.0))
    throughput update Consumed (cons1, 24.0, 27.0,  3.0, Map(op2   ->  3.0))

    throughput update Generated(prod1, 27.0, 30.0, 10.0)
    throughput update Produced (prod1, 30.0, 31.0, 10.0, Map.empty)
    throughput update Produced (op1,   31.0, 41.0, 10.0, Map(prod1 -> 10.0))
    throughput update Produced (op2,   41.0, 44.0,  1.0, Map(op1   ->  1.0))
    throughput update Consumed (cons1, 44.0, 47.0,  1.0, Map(op2   ->  1.0))
  ...
  it should "calculate the right throughput when there are two paths for the same producer" in new Fixture2 {
    ...
    throughput update Produced (prod1, 10.0, 11.0, 10.0, Map.empty)
    throughput update Produced (op1,   11.0, 21.0, 10.0, Map(prod1 -> 10.0))
    throughput update Produced (op2,   21.0, 31.0, 10.0, Map(prod1 -> 10.0))
    throughput update Produced (op3,   31.0, 35.0,  3.0, Map(op1 -> 1.0, op2 -> 2.0))
    throughput update Consumed (cons1, 35.0, 42.0,  3.0, Map(op3 -> 3.0))
  ...
  it should "calculate the right throughput when there are more than one producer" in new Fixture3 {
    ...
    throughput update Generated(prod1,  0.0, 10.0, 10.0)
    throughput update Produced (prod1, 10.0, 20.0, 10.0, Map.empty)
    throughput update Generated(prod2,  0.0, 20.0, 20.0)
    throughput update Produced (prod2, 20.0, 30.0, 20.0, Map.empty)

    throughput update Produced (op1,   30.0, 40.0, 10.0, Map(prod1 -> 10.0))
    throughput update Produced (op2,   40.0, 50.0, 10.0, Map(prod2 -> 10.0))
    throughput update Produced (op3,   50.0, 55.0,  5.0, Map(op1 -> 1.0, op2 -> 4.0))
    throughput update Consumed (cons1, 55.0, 60.0,  5.0, Map(op3 -> 5.0))
  ...
  it should "correctly handle windowed operators" in new Fixture4 {
    ...
    throughput.update(Generated        (prod1,  0.0, 0.0, 10.0))
    throughput.update(Produced         (prod1,  0.0, 1.0, 10.0))
    throughput.update(WindowAccumulated(w1,     1.0, 2.0,    0, Map(prod1 -> 10.0)))

    throughput.update(Generated        (prod1,  2.0, 10.0, 10.0))
    throughput.update(Produced         (prod1, 10.0, 11.0, 10.0))
    throughput.update(WindowAccumulated(w1,    11.0, 12.0, 0, Map(prod1 -> 10.0)))

    throughput.update(Generated        (prod1, 12.0, 20.0, 10.0))
    throughput.update(Produced         (prod1, 20.0, 21.0, 10.0))
    throughput.update(WindowProduced   (w1,    21.0, 22.0, 1.0, 0))
    throughput.update(WindowAccumulated(w1,    21.0, 22.0,   0, Map(prod1 -> 10.0)))
    throughput.update(Produced         (op2,   22.0, 24.0, 1.0, Map(w1  -> 1)))
    throughput.update(Consumed         (cons1, 24.0, 25.0, 1.0, Map(op2  -> 1)))

    throughput.update(Generated        (prod1, 25.0, 40.0, 10.0))
    throughput.update(Produced         (prod1, 40.0, 41.0, 10.0))
    throughput.update(WindowProduced   (w1,    41.0, 42.0, 1.0, 0))
    throughput.update(WindowAccumulated(w1,    41.0, 42.0,    0, Map(prod1 -> 10.0)))
    throughput.update(Produced         (op2,   42.0, 44.0,  1.0, Map(w1   -> 1)))
    throughput.update(Consumed         (cons1, 44.0, 45.0,  1.0, Map(op2  -> 1)))
  ...
  it should "correctly handle window like operators that have the advance duration different from size" in new Fixture4 {
    ...
    throughput.update(Generated        (prod1,  2.0, 10.0, 10.0))
    throughput.update(Produced         (prod1, 10.0, 11.0, 10.0))
    throughput.update(WindowProduced   (w1,    11.0, 12.0,  1.0, 0))
    throughput.update(WindowAccumulated(w1,    11.0, 12.0,  1, Map(prod1 -> 10.0)))
    throughput.update(Produced         (op2,   12.0, 14.0,  1.0, Map(w1  -> 1)))
    throughput.update(Consumed         (cons1, 14.0, 15.0,  1.0, Map(op2 -> 1)))

    throughput.update(Generated        (prod1, 15.0, 20.0, 10.0))
    throughput.update(Produced         (prod1, 20.0, 21.0, 10.0))
    throughput.update(WindowProduced   (w1,    21.0, 22.0,  2.0, 1))
    throughput.update(WindowAccumulated(w1,    21.0, 22.0,    2, Map(prod1 -> 10.0)))
    throughput.update(Produced         (op2,   22.0, 24.0,  2.0, Map(w1  -> 2)))
    throughput.update(Consumed         (cons1, 24.0, 25.0,  2.0, Map(op2 -> 2)))

    throughput.update(Generated        (prod1, 25.0, 30.0, 10.0))
    throughput.update(Produced         (prod1, 30.0, 31.0, 10.0))
    throughput.update(WindowProduced   (w1,    31.0, 32.0,  3.0, 2))
    throughput.update(WindowAccumulated(w1,    31.0, 32.0,    0, Map(prod1 -> 10.0)))
    throughput.update(Produced         (op2,   32.0, 34.0,  3.0, Map(w1  -> 3)))
    throughput.update(Consumed         (cons1, 34.0, 35.0,  3.0, Map(op2 -> 3)))

    // next window only considers new events
    throughput.update(Generated        (prod1, 35.0, 40.0, 10.0))
    throughput.update(Produced         (prod1, 40.0, 41.0, 10.0))
    throughput.update(WindowProduced   (w1,    41.0, 42.0,  3.0, 0))
    throughput.update(WindowAccumulated(w1,    41.0, 42.0,  1, Map(prod1 -> 10.0)))
    throughput.update(Produced         (op2,   42.0, 44.0,  3.0, Map(w1   -> 3)))
    throughput.update(Consumed         (cons1, 44.0, 45.0,  3.0, Map(op2  -> 3)))
