...
class UniformIncreaseGeneratorTest extends FlatSpec
  ...
  it should "give the correct values even when a sample encompasses both regions" in {
    ...
    generator.generate(6000) should be ( 36.0)
    generator.generate(6000) should be (104.0)
    generator.generate(6000) should be (120.0)
  }
  ...
  it should "have an average close to the maxRate if is used for a long time" in {
    val generator = UniformIncreaseGenerator(10 seconds, 20)
    for (i <- 1 to 100000) generator.generate(5000)
