...
class Metric {
  /**
   * Estimate the number of events that needs to be generated by each producer so that the last
   ...
   * @param outputEvents Number of output events to be considered in the calculation.
   * @return An estimation of the total number of events generated by the producers.
   */
  def eventsPerProducer(q: Query, c: EventConsumer, outputEvents: Long): Map[EventProducer, Double] = {
    val pWeights = producersWeight(q)
    eventsPerProducer(q, pWeights, c, outputEvents)
  }
   ...
   * @param c Consumer on which the estimation is executed.
   * @param outputEvents Number of output events to be considered.
   * @return An estimation of the total number of events generated by the producers.
   */
  protected def eventsPerProducer(q: Query, producerWeights: Map[EventProducer, Double], c: EventConsumer, outputEvents: Long):
    ...
    val totalEvents = outputEvents / totalWeight
