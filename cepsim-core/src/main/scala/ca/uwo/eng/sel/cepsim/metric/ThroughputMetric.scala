...
object ThroughputMetric extends Metric {
  ...
  /**
    ...
    * @return Query throughput in events per seconds.
    */
  ...
  /** *
    * Overloaded version of the calculate method which receives the time as a double (in seconds).
    * @param q Query from which the throughput should be calculated.
    * @param totalTime The total execution time of this query (in seconds).
    * @return Query throughput in events per seconds.
    */
  def calculate(q: Query, totalTime: Double): Double = calculate(q, totalTime.seconds)
