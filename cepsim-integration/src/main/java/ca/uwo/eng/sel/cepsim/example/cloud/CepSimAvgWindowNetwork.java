...
public class CepSimAvgWindowNetwork {
	...
	private static Set<Cloudlet> createCloudlets(CepSimBroker broker, int iterations) {
        ...
        for (int i = 1; i <= MAX_QUERIES; i++) {
            ...
            Operator outlierDetector = new Operator("outlierDetector" + i, 12_500, 2048);
            Operator average = WindowedOperator.apply("average" + i, 10_500, 15000, 15000, WindowedOperator.constant(NUM_SENSORS), 2048);
            Operator db = new Operator("db" + i, 10_800_000, 2048);
